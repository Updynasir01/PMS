...
/** Logo for landing customers — keeps PNG transparency when possible */
export async function processLogoBase64(imageBase64, mimeType) {
  if (!imageBase64) throw new Error('Image data required');
  ...
  const sharp = (await import('sharp')).default;
  const pipeline = sharp(buf).resize(320, 120, { fit: 'inside', withoutEnlargement: true });

  if (mime === 'image/png') {
    const out = await pipeline.png({ quality: 90 }).toBuffer();
